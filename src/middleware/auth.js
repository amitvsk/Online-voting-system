...
const bookModel = require('../Models/adminModel');
...
const Authentication = async function (req, res, next) {
    try {
        ...
        jwt.verify(token,"Amit/chandan/majorProject",{ ignoreExpiration: true },function (err, decoded) {
...
const Authorization = async function (req, res, next) {
    try {
        ...
        if (!token) {return res.status(400).send({status:false, msg: "Enter x-api-key In Header" });}
        // verify the token 
        let decodedToken = jwt.verify(token, "Amit/chandan/majorProject")
